...
namespace maplestory
{
	namespace network
	{
		...
		bool tcp_socket::try_connect(std::string const& ip, unsigned short port)
		{
			...
			if (WSAAsyncSelect(this->sock, tcp_handler::get_instance().get_handle(), 
		...
		bool tcp_socket::recv_handshake(packet_reader& packet)
		{
			...
			packet = packet_reader(handshake_buffer, handshake_length);

			delete[] handshake_buffer;
			return true;
		}
		
		bool tcp_socket::recv_packets(std::vector<packet_reader>& packets, unsigned int& iv)
		{
			...
			while (this->recv_offset >= 4)
			{
				...
				packets.push_back(packet_reader(this->recv_buffer + 4, packet_length));
		...
		bool tcp_socket::send_packet(packet_writer& packet, unsigned short version_major, unsigned int& iv)
		{
